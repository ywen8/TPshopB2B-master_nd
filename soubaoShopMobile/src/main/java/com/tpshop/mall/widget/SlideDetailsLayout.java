...
public class SlideDetailsLayout extends ViewGroup {
    ...
    public interface OnSlideDetailsListener {
        void onStatucChanged(Status status);  //Called after status changed
    }
        ...
        switch (aciton) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (canChildScrollVertically((int) yDiff)) {
                    ...
                    if (yDiffabs > mTouchSlop && yDiffabs >= xDiffabs && !(mStatus == Status.CLOSE && yDiff > 0
    ...
    private void processTouchEvent(final float offset) {
        if (Math.abs(offset) < mTouchSlop) return;
        ...
        if (mStatus == Status.CLOSE) {
            if (offset >= 0)
                ...
                mSlideOffset = offset;
            if (mSlideOffset == oldOffset) return;
    ...
    /**
     * Whether the closed pannel is opened at first time.If open first, we should set the behind view's visibility as VISIBLE
     */
    ...
    /**
     * When pulling, target view changed by the panel status. If panel opened, the target is behind view.Front view is for otherwise
     */
    ...
    protected boolean canChildScrollVertically(int direction) {
        ...
        } else if (mTarget instanceof FrameLayout || mTarget instanceof RelativeLayout || mTarget instanceof LinearLayout) {
            View child;
