...
public class ColorArcProgressBar extends View{
    ...
    private String hintColor = "#ffffff";   //已抢文本颜色
    private String currTextColor = "#ffffff";   //当前文本颜色
    ...
    private void initView() {
        ...
        vTextPaint.setColor(Color.parseColor(currTextColor));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) { //内容
            canvas.drawText(String.format("%.0f%%", curValues), centerX, centerY + 2 * textSize / 3 + dipToPx(5), vTextPaint);
        }
       /* if (isNeedUnit) { //单位/步
            canvas.drawText(hintString, centerX, centerY + 2 * textSize / 3, hintPaint);
        }*/
        if (isNeedTitle) { //已抢
            canvas.drawText(titleString, centerX, centerY - 2 * textSize / 3 + dipToPx(5), curSpeedPaint);
        }
